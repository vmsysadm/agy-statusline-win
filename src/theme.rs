...
impl Theme {
    ...
    pub(crate) fn new(data: &InputData, cfg: &Config) -> Self {
        ...
        let use_ascii = data.use_ascii.or(cfg.use_ascii).unwrap_or_else(|| {
            if let Ok(v) = env::var("USE_ASCII") {
                v == "true" || v == "1"
            } else if let Ok(v) = env::var("USE_NERD_FONTS") {
                v == "false" || v == "0"
            } else {
                // No explicit signal: ASCII unless nerd fonts were opted into.
                !cfg.use_nerd_fonts.unwrap_or(false)
            }
        });
        ...
        let icon_waiting = get_icon_str(&cfg_icons, &cfg.icons, cat_state, "waiting", if use_ascii { "~" } else if use_nerd_fonts { "\u{f116b}" } else { "⏳" });
        let icon_thinking = get_icon_str(&cfg_icons, &cfg.icons, cat_state, "thinking", if use_ascii { "?" } else if use_nerd_fonts { "\u{f09d1}" } else { "💭" });
        let icon_working = get_icon_str(&cfg_icons, &cfg.icons, cat_state, "working", if use_ascii { ">" } else if use_nerd_fonts { "\u{f046e}" } else { "⚙" });
        let icon_tool = get_icon_str(&cfg_icons, &cfg.icons, cat_state, "tool", if use_ascii { "=" } else if use_nerd_fonts { "\u{f00ec}" } else { "⚒" });
        ...
        Theme {
            ...
            icon_ready, icon_waiting, icon_thinking, icon_working, icon_tool, icon_unknown,
...
pub(crate) fn get_icon_str(
    ...
    // Look in the active icon set (`icons.<variant>.<category>.<key>`), then fall back to a
    // flat `icons.<key>` map, then the built-in default.
    let lookup = |v: &serde_json::Value| -> Option<String> {
        v.get(category)
    ...
    active_icon_set
        .as_ref()
        .and_then(lookup)
        .or_else(|| raw_icons_cfg.as_ref().and_then(lookup))
