...
# Determine optional log file location
$LOG_FILE = $null
if ($env:STATUSLINE_LOG_FILE) {
    $LOG_FILE = $env:STATUSLINE_LOG_FILE
} elseif ($data -and $data.log_file) {
    $LOG_FILE = $data.log_file
}

# Safe logging function to file
function Write-Log ($message) {
    if (![string]::IsNullOrEmpty($LOG_FILE)) {
        try {
            ...
            $cleanMessage = $message -replace $ansiPattern, ""
            ...
            "[${timestamp}] ${cleanMessage}" | Out-File -FilePath $LOG_FILE -Append -Encoding utf8
        } catch {
            # Suppress failures to ensure status line is uninterrupted
        }
    }
}

# Log initial script execution and input JSON
Write-Log "Execution started."
Write-Log "INPUT JSON: $inputJson"

# --- Config & Path Resolution ------------------------------------------------
$CONFIG_PATH = ""
$SETTINGS_FILE = Join-Path $homePath ".gemini/antigravity-cli/settings.json"
if (Test-Path $SETTINGS_FILE -PathType Leaf) {
    try {
        $settingsJson = Get-Content $SETTINGS_FILE -Raw | ConvertFrom-Json
        if ($settingsJson.statusLine -and $settingsJson.statusLine.configPath) {
            $CONFIG_PATH = $settingsJson.statusLine.configPath
        }
    } catch {}
}
if ([string]::IsNullOrEmpty($CONFIG_PATH)) {
    ...
    $CONFIG_PATH = Join-Path $SCRIPT_DIR "statusline_config.json"
}
...
$all_icons = @(
    ...
    $ICON_CYCLE_ACCEPT, $ICON_CYCLE_PLAN, $ICON_YOLO
)
$unique_chars = @{}
...
# --- Sandbox State & Network Fail-safe Handling ---
...
if (-not $SANDBOX_NET) {
    ...
    if (Test-Path $SETTINGS_FILE -PathType Leaf) {
        try {
            $settingsJson = Get-Content $SETTINGS_FILE -Raw | ConvertFrom-Json
            if ($settingsJson.sandboxAllowNetwork -eq $true) {
                $SANDBOX_NET = $true
            }
        } catch {}
    }
...
function Get-VisibleLength ($str) {
    ...
    return $base_len + ($icon_count * $ICON_WIDTH_ADJUST)
}
...
# Dangerously-skip-permissions badge (YOLO)
if (Test-Path $SANDBOX_LOG -PathType Leaf) {
    try {
        $stream = [System.IO.File]::Open($SANDBOX_LOG, [System.IO.FileMode]::Open, [System.IO.FileAccess]::Read, [System.IO.FileShare]::ReadWrite)
        $reader = [System.IO.StreamReader]::new($stream)
        ...
        if ($logContent -match "dangerously-skip-permissions: auto-approving") {
            $S = "${FG_BRIGHT_RED}${B}${ICON_YOLO} YOLO${R}${DOT}${S}"
        }
    } catch {}
}
...
# Print to standard output and log
